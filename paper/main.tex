...
\textbf{Abstract.}
The proliferation of large language models (LLMs) has created unprecedented opportunities for augmenting scarce human-labeled data with AI-generated annotations. However, existing approaches face fundamental limitations: naive pooling introduces systematic bias, while Prediction-Powered Inference (PPI) corrects for prediction error but requires AI outputs to directly approximate outcomes---a condition violated when AI predictions are categorical or accuracy is moderate. We propose a Double Machine Learning (DML) framework for AI data augmentation in Generalized Linear Models that treats AI predictions as \textit{informative features} rather than outcome surrogates. Our approach constructs Neyman-orthogonal score functions that remain computable even when human labels are missing, enabling valid statistical inference while leveraging both primary (human-labeled) and auxiliary (AI-augmented) data. We establish asymptotic normality and prove that, \textit{under random selection into the primary sample}, our estimator dominates the primary-data-only estimator, achieving strictly smaller asymptotic variance when AI predictions are informative. Experiments on conjoint analysis with GPT-4o annotations demonstrate that DML achieves 16.5\% MAPE, outperforming primary-only estimation (32.0\%), the state-of-the-art AAE method (17.6\%), and PPI (51.1\%), which struggles when its proxy assumption is violated.
\end{minipage}
\vskip 1.5em
\end{center}
\end{@twocolumnfalse}
...
\textbf{Implementation Notes.} Step 3 trains $\hat{g}$ on primary data excluding fold $k$, ensuring out-of-sample predictions. Under random selection, $\hat{e} = n_P/(n_P + n_A)$ is constant (no model needed). Step 5 fits a standard GLM with DML-adjusted targets $\hat{\tau}$, solvable by any convex optimizer.
