...
\begin{itemize}
    ...
    \item We demonstrate DML's effectiveness across prediction quality regimes: on conjoint analysis with noisy LLM predictions, DML achieves 16.5\% MAPE (vs.\ 32.0\% primary-only); on census data with calibrated ML predictions, DML achieves 47.0\% MAPE with 7$\times$ tighter confidence intervals (Section~\ref{sec:experiments}).
\end{itemize}
...
\textbf{Coverage probability.} Table~\ref{tab:conjoint_coverage} reports coverage probability across 30 trials. DML achieves 96.7\% average coverage, closest to the nominal 95\%, indicating well-calibrated confidence intervals. Primary Only (90.0\%) and PPI++ (89.0\%) under-cover, suggesting their standard errors are underestimated.

\begin{table}[t]
\caption{Conjoint benchmark: Coverage probability (\%) for 95\% CI. DML achieves best calibration.}
\label{tab:conjoint_coverage}
\vskip 0.1in
\centering
\small
\begin{tabular}{@{}lccccc@{}}
...
Method & $n$=50 & $n$=100 & $n$=150 & $n$=200 & Avg \\
\midrule
\textbf{DML} & \textbf{97} & \textbf{97} & \textbf{96} & \textbf{97} & \textbf{96.7} \\
...
\subsection{Census Data: Well-Calibrated ML Predictions}
\label{sec:census}

To demonstrate our framework's flexibility across prediction quality regimes, we evaluate on census data where AI predictions are \textit{well-calibrated continuous probabilities} from traditional ML rather than discrete LLM outputs.

\textbf{Data.} We use the California census healthcare dataset ($n = 318,\!215$ individuals) from \citet{angelopoulos2023prediction}. The task is logistic regression predicting healthcare coverage ($y \in \{0,1\}$) from two features: normalized age and income. Ground truth $\bbeta^* = (26.996, -0.120)$ is estimated using all data.

\textbf{AI Predictions.} A gradient boosting classifier provides predicted probabilities $z = P(y=1|X) \in [0,1]$ with 85\% accuracy. Unlike GPT-4o's discrete outputs, these predictions are continuous and well-calibrated: $P(y=1|z=p) \approx p$.

\textbf{Key Methodological Insight: When g=z.} With well-calibrated ML predictions, learning $g(X,z) = \E[y|X,z]$ is unnecessary because calibration implies $\E[y|X,z] \approx z$. We therefore use $g = z$ directly---the AI prediction \textit{is} the conditional expectation. This eliminates nuisance estimation entirely, yielding the simplified DML target:
\begin{equation}
    \tau = z\left(1 - \frac{1}{e}\right) + \frac{w \cdot y}{e}
    \label{eq:tau_census}
\end{equation}

\textbf{Results.} Table~\ref{tab:census} compares methods using confidence intervals with analytical standard errors. We vary labeled samples $m \in \{100, 250, 500, 750, 1000\}$ with $n = 2000$ unlabeled.

\begin{table}[t]
\caption{Census benchmark: MAPE (\%) based on CI midpoint estimates. DML uses $g=z$ directly.}
\label{tab:census}
\vskip 0.1in
\centering
\small
...
\midrule
\textbf{DML ($g$=$z$)} & \textbf{42.0} & \textbf{51.0} & \textbf{63.1} & \textbf{44.3} & \textbf{34.8} & \textbf{47.0} \\
Primary & 76.4 & 206.1 & 81.5 & 108.5 & 111.8 & 116.9 \\
PPI++ & 97.1 & 210.7 & 249.5 & 82.2 & 72.1 & 142.3 \\
\bottomrule
\end{tabular}
\vskip -0.1in
\end{table}
...
\begin{table}[t]
\caption{Census benchmark: Confidence interval width for $\beta_0$ (95\% CI). DML achieves 7$\times$ reduction.}
\label{tab:census_ci}
\vskip 0.1in
\centering
\small
...
\midrule
\textbf{DML ($g$=$z$)} & \textbf{9.88} & \textbf{9.67} & \textbf{8.90} & \textbf{8.39} & \textbf{8.44} \\
...
\textbf{DML achieves 7$\times$ tighter confidence intervals.} Table~\ref{tab:census_ci} shows CI widths for the intercept $\beta_0$. At $m=100$, DML's CI width is 9.88 versus Primary's 71.92---a 7.3$\times$ reduction. This efficiency gain persists across all sample sizes.

\textbf{Standard error comparison.} At $m=100$: $\text{SE}_{\text{DML}}(\beta_0) = 2.52$ versus $\text{SE}_{\text{Primary}}(\beta_0) = 18.35$. At $m=1000$: $\text{SE}_{\text{DML}}(\beta_0) = 2.15$ versus $\text{SE}_{\text{Primary}}(\beta_0) = 3.55$. DML maintains stable precision even at small sample sizes.

\subsection{When to Learn $g$ vs.\ Use $g=z$}
\label{sec:g_choice}

Our two experiments reveal a practical rule for choosing the nuisance model:

\begin{center}
\begin{tabular}{@{}lcc@{}}
\toprule
AI Prediction Quality & Recommended $g$ & Rationale \\
\midrule
Noisy/discrete (LLM, 57\%) & Learn $g(X,z)$ & $z$ unreliable, needs correction \\
Calibrated probabilities (ML, 85\%) & $g = z$ directly & $z \approx \E[y|X,z]$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Why calibration matters.} A well-calibrated predictor $z$ satisfies $\E[y|z=p] \approx p$. Training $g(X,z)$ on limited labeled data introduces estimation variance that exceeds any potential bias reduction. When $z$ is already the optimal prediction of $y$, learning this relationship is redundant.

\textbf{Analogy.} If a weather forecaster says ``70\% chance of rain'' and they're well-calibrated, trying to learn a correction function from limited data will likely make predictions worse, not better.
...
We introduced a paradigm shift in how AI predictions should be used for statistical inference: as \textit{informative features} rather than \textit{outcome proxies}. This perspective, formalized through Double Machine Learning, enables valid inference across the full spectrum of AI prediction quality---from noisy LLM outputs to well-calibrated ML probabilities.

Our theoretical contributions establish asymptotic normality and prove variance dominance over primary-only estimation under random selection. Empirically, DML excels in both regimes tested: on conjoint analysis with noisy LLM predictions (57\% accuracy), DML achieves 16.5\% MAPE versus 32.0\% primary-only---a 48\% relative improvement---with 97\% coverage (closest to nominal 95\%); on census data with well-calibrated ML predictions (85\% accuracy), DML achieves 47.0\% MAPE versus 116.9\% primary-only while reducing confidence interval width by 7$\times$ and maintaining 90\% coverage. Unlike PPI, which struggles with discrete or poorly-calibrated predictions, DML gracefully adapts by either learning the AI-human relationship (noisy predictions) or using predictions directly as conditional expectations (calibrated predictions).
...
\textbf{Variance Estimation: The Clipping Issue.} The DML target for primary data, $\tau = g(1 - 1/e) + y/e$, can take extreme values when $e$ is small. With $n_P = 100$ and $n_A = 1000$, we have $e \approx 0.09$ and $1/e \approx 11$. When $y = 1$ and $g = 0.2$, the unclipped $\tau \approx 11$. For optimization stability, $\tau$ is typically clipped to $[0, 1]$, but this destroys variance information: the residual $\tau - p$ shrinks from $\approx 10$ to $\approx 0.5$, making the sandwich SE estimator severely underestimate uncertainty. The solution is to use \textit{unclipped} $\tau$ for variance estimation while keeping clipped $\tau$ for optimization. This preserves the full variance information in the influence function, yielding well-calibrated confidence intervals (97\% coverage vs.\ 52\% with clipped $\tau$).
...
\section{Census Data Experiment Details}
\label{app:census}

\subsection{Dataset Description}

The census healthcare dataset originates from the 2019 American Community Survey (California) and is publicly available through the PPI package \citep{angelopoulos2023prediction}. Key statistics:

\begin{itemize}
    \item \textbf{Total samples}: $n = 318,215$ individuals
    \item \textbf{Outcome}: Healthcare coverage indicator $y \in \{0, 1\}$
    \item \textbf{Features}: Age (normalized to $[0,1]$) and income (normalized by maximum)
    \item \textbf{Base rate}: Approximately 90\% have healthcare coverage
\end{itemize}

\subsection{AI Prediction Generation}

AI predictions $z = P(y=1|X)$ are generated using gradient boosting:
\begin{enumerate}
    \item Train XGBoost classifier on 80\% of data
    \item Predict probabilities on held-out 20\%
    \item Predictions achieve 85\% accuracy with well-calibrated probabilities
\end{enumerate}

The calibration property $\E[y|z=p] \approx p$ is critical for the $g=z$ simplification. We verify this empirically by binning predictions and checking that the observed rate matches the predicted probability within each bin.

\subsection{DML Implementation with $g=z$}

When AI predictions are well-calibrated continuous probabilities, learning $g(X,z)$ introduces unnecessary estimation variance. We use $g = z$ directly, yielding the implementation:

\begin{enumerate}
    \item \textbf{Compute propensity}: $\hat{e} = m / (m + n)$ where $m$ = labeled, $n$ = unlabeled
    \item \textbf{Compute DML targets}:
    \begin{itemize}
        \item Labeled: $\tau_i = z_i(1 - 1/\hat{e}) + y_i/\hat{e}$, clipped to $[0, 1]$
        \item Unlabeled: $\tau_i = z_i$
    \end{itemize}
    \item \textbf{Fit logistic regression}: Minimize $\sum_i [-\tau_i \cdot X_i\beta + \log(1 + \exp(X_i\beta))]$
    \item \textbf{Compute standard errors}: Via sandwich estimator (see below)
\end{enumerate}

\textbf{Optimization.} We use L-BFGS-B with tolerance $10^{-12}$ for reliable convergence. Initial experiments with Adam optimizer showed convergence issues on this dataset due to the large coefficient magnitudes ($\beta_0 \approx 27$).

\subsection{Standard Error Estimation}

For analytical confidence intervals, we compute standard errors via the sandwich estimator:
\begin{equation}
    \hat{\mathbf{\Sigma}} = \hat{\mathbf{J}}^{-1} \hat{\mathbf{M}} \hat{\mathbf{J}}^{-1} / n
\end{equation}
where:
\begin{align}
    \hat{\mathbf{J}} &= \frac{1}{n} \sum_{i=1}^n X_i^\top \hat{p}_i(1-\hat{p}_i) X_i \quad \text{(bread)} \\
    \hat{\mathbf{M}} &= \frac{1}{n} \sum_{i=1}^n \hat{\psi}_i \hat{\psi}_i^\top \quad \text{(meat)} \\
    \hat{\psi}_i &= X_i^\top (\tau_i - \hat{p}_i) \quad \text{(influence function)}
\end{align}
with $\hat{p}_i = 1/(1 + \exp(-X_i\hat{\beta}))$.

The 95\% confidence interval for $\beta_j$ is:
\begin{equation}
    \hat{\beta}_j \pm 1.96 \cdot \sqrt{\hat{\Sigma}_{jj}}
\end{equation}

\subsection{Comparison with PPI/PPI++}

Our implementation of PPI and PPI++ uses the official \texttt{ppi\_py} package with default settings. Key differences from DML:

\begin{itemize}
    \item \textbf{PPI}: Uses $\lambda = 1$ fixed, computes rectified estimator
    \item \textbf{PPI++}: Automatically tunes $\lambda$ to minimize variance
    \item \textbf{DML}: No $\lambda$ parameter; uses $g=z$ directly in score function
\end{itemize}

Results are consistent with the original PPI paper when using identical seeds and data splits, validating our implementation.

\subsection{Why PPI/PPI++ Underperforms on Census Data}

Despite well-calibrated predictions, PPI/PPI++ achieves higher MAPE than DML due to:

\begin{enumerate}
    \item \textbf{Rectification variance}: PPI's correction term $\frac{1}{m}\sum_{i \in \text{labeled}}(y_i - z_i)$ has variance $O(1/m)$, which dominates at small $m$.

    \item \textbf{Unstable $\lambda$ tuning}: PPI++'s variance-minimizing $\lambda$ is estimated from data, introducing additional uncertainty at small sample sizes.

    \item \textbf{Coefficient magnitude}: Ground truth $\beta_0 \approx 27$ means small relative errors in estimation translate to large absolute errors, amplifying the effect of rectification variance.
\end{enumerate}

DML avoids these issues by incorporating AI predictions directly into the optimization objective rather than as a post-hoc correction.

\subsection{Reproducibility}

All experiments use:
\begin{itemize}
    \item \texttt{numpy.random.RandomState(seed)} for reproducibility
    \item Seeds 0--4 for the five sample sizes in Table~\ref{tab:census}
    \item L-BFGS-B optimizer with \texttt{ftol=1e-12}, \texttt{gtol=1e-12}
    \item Data normalization: age to $[0,1]$, income divided by maximum
\end{itemize}

Code is available at \url{https://github.com/anonymous/dml-augmentation}.
