...
\textbf{Abstract.}
The proliferation of large language models (LLMs) has created unprecedented opportunities for augmenting scarce human-labeled data with AI-generated annotations. However, existing approaches face fundamental limitations: naive pooling introduces systematic bias, while Prediction-Powered Inference (PPI) assumes AI outputs serve as direct proxies for outcomes---an assumption violated when AI accuracy is moderate. We propose a Double Machine Learning (DML) framework for AI data augmentation in Generalized Linear Models that treats AI predictions as \textit{informative features} rather than outcome surrogates. Our approach constructs Neyman-orthogonal score functions that remain computable even when human labels are missing, enabling valid statistical inference while leveraging both primary (human-labeled) and auxiliary (AI-augmented) data. We establish asymptotic normality and prove that our estimator \textit{dominates} the primary-data-only estimator under random selection, achieving strictly smaller asymptotic variance when AI predictions are informative. Extensive experiments on conjoint analysis with GPT-4o annotations demonstrate that DML reduces estimation error by 48\% compared to primary-only estimation (equivalent to \$2.41 in willingness-to-pay accuracy per attribute), while PPI fails catastrophically with 394\% MAPE due to violated assumptions.
\end{minipage}
\vskip 1.5em
\end{center}
\end{@twocolumnfalse}
...
The emergence of large language models (LLMs) has fundamentally transformed the economics of data collection. Models like GPT-4 can annotate text, answer surveys, and produce labels at costs orders of magnitude lower than human annotation \citep{brown2020language, openai2023gpt4}. This capability is transformative for empirical research in domains where human labels are expensive: conjoint analysis in marketing requires \$5--20 per respondent \citep{sawtooth2017conjoint}, medical image annotation costs \$50--500 per case, and legal document review runs \$25--150 per hour. When LLM API calls cost \$0.01--0.10 per annotation, the potential for cost savings is enormous.
...
\begin{itemize}
    ...
    \item We demonstrate through extensive experiments on conjoint analysis that DML achieves 16.5\% MAPE, improving over primary-only (32.0\%), AAE (17.6\%), and dramatically over PPI (51.1\% after filtering abstentions; Section~\ref{sec:experiments}).
\end{itemize}
...
\textbf{Prediction-Powered Inference.} \citet{angelopoulos2023prediction} introduced PPI for constructing valid confidence intervals using ML predictions. The key insight is that labeled data can correct for systematic prediction errors. However, PPI assumes predictions directly approximate outcomes. When this fails---as with categorical AI outputs or moderate accuracy---PPI's variance correction can explode. Our framework instead learns $g(\bX,\bz) = \E[\by|\bX,\bz]$, using $\bz$ as a conditioning variable rather than assuming $\bz \approx \by$.

\textbf{Double Machine Learning.} \citet{chernozhukov2018double} developed DML for causal inference with high-dimensional nuisance parameters. The Neyman orthogonality condition ensures that estimation error in nuisance functions has only second-order effects on the target parameter. We adapt this framework to the AI augmentation setting, where the nuisance function $g(\bX,\bz) = \E[\by|\bX,\bz]$ captures the information content of AI predictions.

\textbf{Semi-Supervised and Transfer Learning.} Classical semi-supervised methods \citep{chapelle2006semi, zhu2009introduction} use unlabeled features to improve prediction. Transfer learning \citep{pan2009survey, weiss2016survey} addresses distribution shift between domains. Our setting is distinct: we have AI-generated \textit{labels} (not just features) for auxiliary data, but these labels may be systematically biased. The AAE framework \citep{wang2024llm} addresses this through parametric debiasing; we provide a nonparametric alternative.

\textbf{Missing Data.} Our framework connects to missing data theory \citep{little2019statistical, rubin1976inference}, where human labels are ``missing'' for auxiliary observations. The key distinction is that we observe AI predictions $\bz$ for all observations, providing rich auxiliary information beyond the missingness indicator.
...
\label{alg:dml}
\begin{algorithmic}[1]
\REQUIRE Data $\mathcal{D}$, folds $K$
\STATE Partition $\mathcal{D}$ into $K$ folds $I_1, \ldots, I_K$
\FOR{$k = 1, \ldots, K$}
    \STATE Train $\hat{e}, \hat{g}$ on $\mathcal{D} \setminus I_k$ (primary data only for $\hat{g}$)
    \STATE Solve for $\hat{\bbeta}_k$: $\left\|\frac{1}{|I_k|}\sum_{i \in I_k} \bpsi(\Xi_i; \hat{e}, \hat{g}; \hat{\bbeta}_k)\right\| \leq \inf_\bbeta \|\cdot\| + o_P(n^{-1/2})$
\ENDFOR
\RETURN $\hat{\bbeta} = \frac{1}{K}\sum_{k=1}^K \hat{\bbeta}_k$
\end{algorithmic}
\end{algorithm}

Cross-fitting (Step 3) ensures that nuisance estimates $\hat{e}, \hat{g}$ are independent of the data used for inference in each fold, which is essential for valid asymptotic theory.
...
The product rate condition $r_1 + r_2 \geq 1/2$ is standard in DML and satisfied by many ML methods including random forests, neural networks, and boosting when the underlying functions have sufficient smoothness.
...
\textbf{Interpretation.} The strict inequality condition holds whenever AI predictions are \textit{informative}---i.e., $g^*(\bX,\bz) \neq \nabla b(\bX\bbeta^*)$. When AI predictions perfectly match the GLM's conditional mean, DML and primary-only achieve identical efficiency. Otherwise, DML strictly dominates.
...
\begin{table}[t]
\caption{Sports car conjoint attributes and ground truth part-worths (willingness-to-pay in \$1000s relative to base level)}
...
\textbf{AI Predictions.} We prompt GPT-4o with each choice task and record its prediction $z \in \{-1, 0, 1\}$: choose option 1 ($z=1$), choose option 2 ($z=0$), or abstain ($z=-1$). GPT-4o achieves 57\% accuracy (excluding abstentions), reflecting the genuine difficulty of predicting human preferences.
...
\begin{enumerate}
    ...
    \item \textbf{DML-2}: Variant averaging fold-specific $\hat{\bbeta}_k$ (exact algorithm from Section~\ref{sec:method})
...
Table~\ref{tab:main} presents Mean Absolute Percentage Error (MAPE) across methods.
...
DML-2 & 17.6 & 16.9 & 16.5 & 15.9 & 16.7 \\
...
In conjoint analysis, parameters represent willingness-to-pay (WTP) for attribute improvements. Table~\ref{tab:wtp} translates MAPE into economic terms.
...
With average true WTP of \$3.5K per attribute, DML's 16.5\% MAPE translates to \$580 error per attribute. Primary Only's 32.0\% MAPE means \$1,120 error---nearly double. For a product with 11 attributes, this compounds to \$6,380 vs.\ \$12,320 total error, potentially leading to substantially different pricing and product design decisions.
...
\begin{itemize}
    \item \textbf{PPI}: Assumes $\bz \approx \by$ and corrects for $\bz - \by$ using labeled data. When AI accuracy is low, the correction term has high variance.

    \item \textbf{DML}: Learns $g(\bX, \bz) = \E[\by | \bX, \bz]$, treating $\bz$ as a feature. Even when $\bz$ poorly predicts $\by$ marginally, it may be informative conditionally---e.g., GPT-4o may systematically prefer certain attribute combinations, providing signal about human preferences.
\end{itemize}

Figure~\ref{fig:comparison} (Appendix~\ref{app:figures}) shows that DML's advantage grows as AI accuracy decreases, while PPI degrades rapidly below 70\% accuracy.
...
We developed a Double Machine Learning framework for AI data augmentation in Generalized Linear Models. The key insight is treating AI predictions as informative features rather than outcome proxies, enabling valid inference even when AI accuracy is moderate.

Our theoretical contributions establish asymptotic normality and prove variance dominance over primary-only estimation. Empirically, DML achieves state-of-the-art performance on conjoint analysis, reducing estimation error by 48\% versus primary-only and avoiding the catastrophic failures of PPI when its assumptions are violated.
...
DML-2 & 17.56 & 16.93 & 16.46 & 15.94 & 16.72 & 0.70 \\
...
[Placeholder for figures showing:
\begin{itemize}
    \item MAPE vs.\ AI accuracy for DML vs.\ PPI
    \item Convergence of DML estimates with increasing $n_P$
    \item Calibration plots for g-model predictions
\end{itemize}]
